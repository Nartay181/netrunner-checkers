...
export function AuthPanel({ auth, open }: AuthPanelProps) {
  ...
  function pushLog(log: string) {
    setTerminalLogs((current) => [log, ...current].slice(0, 6));
  }
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="grid gap-3">
              ...
              <motion.button
                ...
                className="mt-1 inline-flex items-center justify-center gap-2 rounded-md border border-matrix/50 bg-matrix/10 px-4 py-3 text-xs font-black uppercase text-matrix shadow-matrix-soft transition disabled:cursor-not-allowed disabled:opacity-55"
              >
                {auth.loading ? (
                ...
                {auth.loading ? "AUTHENTICATING" : title}
              </motion.button>
            </form>

            <div className="mt-5 grid gap-2">
              {(auth.error ? [`[AUTH_FAILURE]: ${auth.error}`] : [])
                .concat(terminalLogs)
                .map((log, index) => (
                  <motion.p
                    key={`${log}-${index}`}
